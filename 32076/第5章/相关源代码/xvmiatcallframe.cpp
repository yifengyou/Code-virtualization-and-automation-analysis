...
void xVMIATCallFrame::saveRecords(TCHAR *filename)
{
	...
	NPacketBuffer pkmods;
	NPacketBuffer pkmodone;
	NPacketBuffer pkfunc;
...
void xVMIATCallFrame::loadRecords(TCHAR *filename)
{
    ...
    m_mods.clear();
    m_calls.clear();
	NPacketBuffer pk(lpdata,szdata,false);
	int pos = 0;
	int vid;
	int szblock;
	for (const char* lpnode = pk.tonext(pos, -1, &vid, &szblock); lpnode; lpnode = pk.tonext(pos, -1, &vid, &szblock))
    {
        switch(vid)
        {
        case iatcall_vid_module:
        {
			NPacketBuffer pkmods(lpnode,szblock,false);
			int posmod = 0;
			int vidmod = 0;
			int szblockmod;

			for (const char* lpmod = pkmods.tonext(posmod, -1, &vidmod, &szblockmod); lpmod; lpmod = pkmods.tonext(posmod, -1, &vidmod, &szblockmod))
            {
				NPacketBuffer pkmodone(lpmod,szblockmod,false);
				...
				int vidone = 0;
				int szblockone = 0;
				for (const char* lpone = pkmodone.tonext(posone, -1, &vidone, &szblockone); lpone; lpone = pkmodone.tonext(posone, -1, &vidone, &szblockone))
                {
                    switch(vidone)
                    {
                    case iatcall_vid_node:
                    {
						NPacketBuffer pknode(lpone,szblockone,false);
                        iat_func func;
	...
	for (const char* lpnode = pk.tonext(pos, -1, &vid, &szblock); lpnode; lpnode = pk.tonext(pos, -1, &vid, &szblock))
	{
        ...
        {
			NPacketBuffer pkcall(lpnode,szblock,false);
            iatcall_info info;
