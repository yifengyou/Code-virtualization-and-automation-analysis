...
void loadxvmcfg(NPacketBuffer* pkopt,xvmdbg_config* cfg)
{
	...
	for (const char* lpd = pkopt->tonext(optpos,xvmdbg_vid_recentpasm); lpd; lpd = pkopt->tonext(optpos,xvmdbg_vid_recentpasm))
	{
	...
	cfg->idasvr = pkopt->getWString(xvmdbg_vid_idasvrname, L".");
}



void savexvmcfg(NPacketBuffer* pkopt,xvmdbg_config* cfg)
{
	...
	pkopt->putWString(xvmdbg_vid_idasvrname, cfg->idasvr.c_str());
...
int loadxvmdbg_config(xvmdbg_config* cfg, const TCHAR *file)
{
    ...
    {
		NPacketBuffer pkopt(lpdata,szdata,false);
        loadxvmcfg(&pkopt,cfg);
    }
    return 0;
}
