...
#include "../../../3rdparty/nedmalloc/nedmalloc_i.h"
extern "C"{
	void*	nc_malloc(size_t size)
	{
		return (void*)nedalloc::nedmalloc(size);
	}
	...
	{
		return (void*)nedalloc::nedrealloc(ptr, size);
	}
}
...
int SendPacketByCurrentThread(NPacketBuffer* pk,BOOL singleThread = FALSE)
{
...
//脚本参数打包
void genRunScriptPacket(const char* filename, NPacketBuffer* pkopt)
{
...
{	//包装封包
	NPacketBuffer pk(xvm_notice_runscript);
	NPacketBuffer pkopt;
...
void ProcessDbgFuncCall(NPacketBuffer* pk)
{
	switch (pk->getMsgID())
	{
	...
	while (gNoticePipe.fpPeek(NULL,0,NULL,0,&dwByte) && dwByte > 0)
	{
		...
		case xvm_notice_blocks:{
			...
			if (pk.getBinary(xvm_vid_bkfile,lpdata,szdata))
			{
				...
				NPacketBuffer pkbody(lpdata,szdata,false);
		...
		case xvm_notice_records:
		{
			if (gECpuFrame)
			{
				...
				int pos = 0;
				int vid;
				for (const char* lprc = pk.tonext(pos, -1, &vid); lprc; lprc = pk.tonext(pos, -1, &vid))
				{
					switch (vid)
					{
...
void ProcessIDAPacket()
{
	...
	while (gIDANotice.fpPeek(NULL,0,NULL,0,&dwByte) && dwByte > 0)
	{
		...
		case xida_vid_syncip:{
			ULONG_PTR edr = pk.getInt(xida_vid_address,0);
...
void CheckRemoteIDAConnect()
{
	...
	for (int i = 0; i < MAX_IDA_INST; i++)
	{
		...
		{
			TCHAR buf[MAX_PATH];
			wsprintfW(buf,REMOTEIDA_PIPE_TEMP,gxvmcfg.idasvr.c_str(),i);
			pi->pipe.SetPipeName(buf);
			if (pi->pipe.OpenSyncInst())
			{
				xida_msg_getinfo msgi;
				msgi.pid = GetCurrentProcessId();
				DWORD szn = sizeof(msgi.computerName);
				GetComputerNameA(msgi.computerName, &szn);
...
{
	NPacketBuffer pk(xvm_notice_entervm);
	NPacketBuffer pkopt;
...
{
	NPacketBuffer pk(xvm_notice_dbgdirect);
...
void resetIDAPipe()
{
	if (gxvmcfg.bRemoteIDA)
	{
		gIDANotice.fpClose();
		TCHAR name1[64];
		_stprintf(name1,REMOTEIDA_PIPE_NOTICE,_T("."),GetCurrentProcessId());
...
void saveOptions(HWND hwndDlg)
{
	...
	GetDlgItemTextW(hwndDlg, IDC_IDASERVER, ws, MAX_PATH);
	gxvmcfg.idasvr = ws;
        ...
        )
{
    ...
    case WM_INITDIALOG:
    {
		...
		SetDlgItemTextW(hwndDlg, IDC_IDASERVER, gxvmcfg.idasvr.c_str());
    }return TRUE;
    case WM_COMMAND:
        switch (LOWORD(wParam))
        {
