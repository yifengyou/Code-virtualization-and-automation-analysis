...
int xVMBlockFrame::loadBlocks(NPacketBuffer* pk)
{
	m_seqs.clear();
	m_instIndex.clear();
	m_sseq.clear();
	...
	for (const char* lpnode = pk->tonext(pos, -1, &vid, &szblock); lpnode; lpnode = pk->tonext(pos, -1, &vid, &szblock))
	{
		...
		{
			NPacketBuffer pkseq(lpnode,szblock,false);
			...
			int vidseq = 0;
			int szblockseq = 0;
			for (const char* lptr = pkseq.tonext(posseq, -1, &vidseq, &szblockseq); lptr; lptr = pkseq.tonext(posseq, -1, &vidseq, &szblockseq))
			{
				if (vidseq == block_vid_record)
				{
					if (szblockseq == sizeof(TraceRecordEx))
		...
		{
			m_sseq.push_back(m_sseq.count(),*(SequenceExecute*)lpnode);
		}break;
		case block_vid_tpl:
		{
			opexec_tpl* tpl =(opexec_tpl*)lpnode;
